...
// tracklist rows — rendered from the album data before the webgl pass reads them
function renderTracklist() {
  const container = document.querySelector(".tracklist-items .container");
  if (!container) return;

  container.innerHTML = TRACKS.map((track, index) => {
    const feat = track.feat ? ` <span>(feat. ${track.feat})</span>` : "";
    return `
      <div class="tracklist-item-row">
        <a href="${trackUrl(track)}">
          <div class="tracklist-item">
            <img src="${trackArt(index)}" alt="${track.title} — ${t(track.tagline)}" />
            <div class="tracklist-item-info">
              <p>${trackNumber(index)} — ${track.title}${feat}</p>
              <p>${track.length}</p>
            </div>
          </div>
        </a>
...
class TracklistDistortion {
  ...
  createMeshes() {
    ...
    const media = [...document.querySelectorAll(".tracklist-item img")];
...
// initialization
document.addEventListener("DOMContentLoaded", () => new TracklistDistortion());
